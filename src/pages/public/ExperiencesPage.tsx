...
function ExperienceCard({ experience }: { experience: Experience }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-blue-100 bg-white transition duration-200 hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5">
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50/60 p-4">
          ...
          <p className="line-clamp-3 font-mono text-xs leading-5 text-slate-700">
            {experience.cause}
          </p>
        </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {experience.technologie
              ...
              .map((technologie) => (
                <span
                  key={technologie}
                  className="rounded-md border border-slate-100 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-600"
                >
                  {technologie}
                </span>
              ))}
        ...
        <div className="mt-auto pt-6">
          ...
          <Link
            ...
            className="mt-4 flex items-center justify-between rounded-xl border border-blue-200 bg-white px-4 py-3 text-sm font-semibold text-blue-600 transition hover:border-blue-600 hover:bg-blue-600 hover:text-white"
          >
...
export default function ExperiencesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      {/* En-tête */}
      <section className="border-b border-blue-100 pb-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50/80 px-3 py-1.5 text-xs font-semibold text-blue-700">
              <BookOpen size={14} />
            ...
            <p className="mt-4 text-sm leading-6 text-white sm:text-base">
              Découvrez des problèmes techniques réels, comprenez leurs causes et
              consultez les solutions proposées par la communauté CodeDoctor.
            </p>
          </div>

          <Link
            to="/dashboard"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700"
          >
            <Sparkles size={16} />
        ...
        <div className="flex flex-col gap-3 lg:flex-row">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="h-12 w-full rounded-xl border border-blue-100 bg-white pl-11 pr-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-600 focus:ring-4 focus:ring-blue-500/10"
            />
          </div>
          ...
          <button
            ...
            className="h-12 rounded-xl bg-blue-600 px-6 text-sm font-semibold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700"
          >
            Rechercher
          </button>
        </div>
        ...
        {chargement && (
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-390px animate-pulse rounded-2xl border border-blue-100 bg-white p-6"
              >
                <div className="h-6 w-24 rounded bg-blue-50" />
                <div className="mt-6 h-6 w-4/5 rounded bg-blue-50" />
                <div className="mt-3 h-4 w-full rounded bg-blue-50" />
                <div className="mt-2 h-4 w-5/6 rounded bg-blue-50" />
        ...
        {!chargement && !erreur && experiences.length === 0 && (
          <div className="rounded-2xl border border-dashed border-blue-200 bg-white px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-white">
              Aucune expérience ne correspond actuellement à vos critères de
              recherche.
            </p>
            ...
            <button
              ...
              className="mt-5 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700"
            >
      ...
      {!chargement && !erreur && experiences.length > 0 && pages > 1 && (
        <nav
          className="mt-8 flex items-center justify-between border-t border-blue-100 pt-6"
        ...
        >
          <button
            ...
            disabled={page <= 1}
            className="inline-flex h-10 items-center gap-2 rounded-lg border border-blue-100 bg-white px-3.5 text-sm font-semibold text-slate-600 transition hover:border-blue-300 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <ChevronLeft size={16} />
          ...
          <span className="text-sm text-slate-500">
            Page{" "}
            <strong className="font-bold text-slate-900">{page}</strong> sur{" "}
