  ...
  const chargerPaiements = useCallback(
    async (pageDemandee: number) => {
      try {
        ...
        const resultat = await obtenirPaiementsAdmin(pageDemandee, limite);
        ...
        setErreur("Impossible de récupérer les demandes de paiement.");
      } finally {
        setChargement(false);
      }
    },
 ...
 useEffect(() => {
  ...
  const paiementsFiltres = useMemo(() => {
    ...
    return paiements.filter((paiement) => {
      return (
        paiement.user.email.toLowerCase().includes(terme) ||
        (paiement.user.displayName ?? "").toLowerCase().includes(terme) ||
        paiement.statut.toLowerCase().includes(terme)
      );
    });
  }, [paiements, recherche]);
  ...
  async function changerStatutPaiement(
    ...
    try {
      ...
      const resultat =
        action === "APPROVE"
          ? await approuverPaiementAdmin(paiement.id)
          : await rejeterPaiementAdmin(paiement.id);

      setPaiements((anciensPaiements) =>
        anciensPaiements.map((ancienPaiement) =>
  ...
  function libelleStatut(statut: StatutPaiementAdmin) {
    switch (statut) {
      case "PENDING":
        return "En attente";
      case "APPROVED":
  ...
  function BadgeStatut({ statut }: { statut: StatutPaiementAdmin }) {
    if (statut === "PENDING") {
      return (
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
            Consultez et traitez les demandes d'activation Premium envoyées par les utilisateurs.
          </p>
        </div>

        <button
          type="button"
          onClick={() => chargerPaiements(page)}
          disabled={chargement || paiementEnModification !== null}
      ...
      <Card className="p-4">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="h-11 w-full rounded-xl border border-zinc-200 bg-white px-4 text-sm text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-zinc-900 focus:ring-4 focus:ring-zinc-900/5"
          />
        ...
        {recherche && (
          <p className="mt-3 text-xs text-zinc-400">
            {paiementsFiltres.length} résultat{paiementsFiltres.length > 1 ? "s" : ""} sur cette page.
          </p>
        )}
      </Card>
      ...
      {chargement && paiements.length === 0 ? (
        <Card className="p-12">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="mt-4 text-sm font-medium text-zinc-700">Chargement des paiements...</p>
            <p className="mt-1 text-xs text-zinc-400">Récupération des demandes depuis le serveur.</p>
          </div>
        </Card>
      ) : (
          ...
          <div className="divide-y divide-zinc-100 md:hidden">
            {paiementsFiltres.length === 0 ? (
              <div className="p-10 text-center">
                <Smartphone size={32} className="mx-auto text-zinc-300" />
                <p className="mt-3 text-sm font-medium text-zinc-700">Aucun paiement trouvé</p>
              </div>
            ) : (
              paiementsFiltres.map((paiement) => {
                const modificationEnCours = paiementEnModification === paiement.id;

                return (
                  <div key={paiement.id} className="space-y-5 p-5">
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold text-zinc-900">
                            {paiement.user.displayName || "Utilisateur sans nom"}
                          </p>
                          <p className="mt-0.5 truncate text-xs text-zinc-500">{paiement.user.email}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="rounded-xl bg-zinc-50 p-3">
                        <p className="text-[11px] text-zinc-400">Montant</p>
                      ...
                      <div className="rounded-xl bg-zinc-50 p-3">
                        <p className="text-[11px] text-zinc-400">Méthode</p>
                        <p className="mt-1 text-sm font-semibold text-zinc-900">WhatsApp</p>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 border-t border-zinc-100 pt-4 text-xs text-zinc-400">
                      <Clock3 size={13} />
                      Demande du {formaterDate(paiement.createdAt)}
                    </div>
                    ...
                    {paiement.statut === "PENDING" && (
                      <div className="grid grid-cols-2 gap-2 border-t border-zinc-100 pt-4">
                        <button
                          ...
                          onClick={() => void changerStatutPaiement(paiement, "REJECT")}
                        ...
                        <button
                          type="button"
                          disabled={modificationEnCours}
                          onClick={() => void changerStatutPaiement(paiement, "APPROVE")}
            ...
            <table className="w-full min-w-237.5">
              <thead className="border-b border-zinc-100 bg-zinc-50/70">
                <tr className="text-left">
                  ...
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Montant</th>
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Méthode</th>
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Statut</th>
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Demande</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {paiementsFiltres.length === 0 ? (
                  ...
                  paiementsFiltres.map((paiement) => {
                    ...
                    return (
                      <tr key={paiement.id} className="transition hover:bg-zinc-50/70">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="max-w-50 truncate text-sm font-semibold text-zinc-900">
                                {paiement.user.displayName || "Sans nom"}
                              </p>
                              <div className="mt-1 flex items-center gap-1.5">
                                <Mail size={12} className="text-zinc-400" />
                                <p className="max-w-55 truncate text-xs text-zinc-500">{paiement.user.email}</p>
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <p className="text-sm font-semibold text-zinc-900">
                            {formaterMontant(paiement.montant)} FCFA
                          </p>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                            <Clock3 size={13} />
                            {formaterDate(paiement.createdAt)}
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          {paiement.statut === "PENDING" ? (
                            ...
                            <span className="text-xs text-zinc-400">Traité</span>
                          )}
                        </td>
                      </tr>
                    );
