        ...
        <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-700/70 sm:text-base">
              Consultez les notifications liées à votre espace administrateur et gérez leur état.
            </p>
          </div>
      ...
      {chargement ? (
        ...
        <div className="space-y-3">
          {notifications.map((notification) => (
            <Card
              key={notification.id}
              className={`border-blue-100 p-4 transition sm:p-5 ${
                notification.lue
                  ? "bg-white"
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {!notification.lue && (
                      <button
                        type="button"
                        onClick={() => void marquerCommeLue(notification.id)}
                        disabled={actionEnCours === notification.id}
                    ...
                    <button
                      type="button"
                      onClick={() => void supprimer(notification.id)}
                      disabled={actionEnCours === notification.id}
                      className="inline-flex items-center gap-2 rounded-lg border border-red-100 bg-white px-3 py-2 text-xs font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <Trash2 size={14} />
                      Supprimer
                    </button>
